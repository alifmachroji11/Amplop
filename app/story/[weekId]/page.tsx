...
export default async function StoryPage(props: PageProps<'/story/[weekId]'>) {
  ...
  const [{ data: weekData }, { data: allData }] = await Promise.all([
    withOwner(supabase.from('transactions').select('*'), auth)
      .gte('occurred_at', start.toISOString().slice(0, 10))
      .lte('occurred_at', end.toISOString().slice(0, 10)),
    withOwner(supabase.from('transactions').select('*'), auth).order('occurred_at', { ascending: false }),
  ]);

  const transactions = (weekData ?? []) as Transaction[];
  const allTransactions = (allData ?? []) as Transaction[];
  ...
  const transactionsPanel = (
    <div>
      ...
      <TransactionsList initial={allTransactions} />
    </div>
  );
  ...
  return (
    <PhoneShell active="story">
      <div className="min-h-[800px] px-6.5 pt-9 pb-12.5 lg:min-h-[520px] lg:max-w-[600px] lg:px-11 lg:pt-11 lg:pb-11">
        <StoryTabs storyPanel={storyPanel} transactionsPanel={transactionsPanel} />
      </div>
    </PhoneShell>
  );
}
